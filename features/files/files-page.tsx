...
import { Archive, Download, FileCode, FileText, Image, Trash2, Upload } from 'lucide-react'
import { CreateEntityDialog, type FileFormValues } from '@/components/shared/create-entity-dialog'
...
const typeColor: Record<string, string> = {
  ...
  file: 'text-muted-foreground bg-secondary',
}
...
export function FilesPage() {
  ...
  const loadFiles = useCallback(async () => {
    const data = await fetchJson<FileRecord[]>('/api/files')
    setFiles(data)
  }, [])
  ...
  const handleUploadFile = async (payload: FileFormValues) => {
    setError(null)
    try {
      const body = new FormData()
      body.set('file', payload.file)
      body.set('category', payload.category)
      body.set('clientId', payload.clientId)
  ...
  const handleDeleteFile = async (file: FileRecord) => {
    const confirmed = window.confirm(`"${file.name}" kaydini silmek istiyor musunuz?`)
    if (!confirmed) {
      return
    }
    ...
    <div className="p-6 space-y-5 overflow-y-auto h-full">
      <PageHeader
        ...
        action={
          <CreateEntityDialog
            entity="file"
            trigger={<Button size="sm" className="h-8 text-sm bg-primary text-primary-foreground hover:bg-primary/90 gap-1.5"><Upload className="w-3.5 h-3.5" />Yükle</Button>}
            onFileSubmit={handleUploadFile}
          />
        }
      />
        ...
        <div className="flex gap-3 overflow-x-auto pb-1">
          {!isLoading &&
            files.slice(0, 4).map((file) => {
              ...
              return (
                <div key={file.id} className="bg-card border border-border rounded-xl p-3 min-w-[180px] hover:border-border/80 transition-colors">
            ...
            <tbody>
              {!isLoading &&
                filtered.map((file, index) => {
                  ...
                  return (
                    <tr key={file.id} className={cn('border-b border-border/50 hover:bg-secondary/40 transition-colors', index === filtered.length - 1 && 'border-0')}>
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-1">
                          ...
                          <button className="p-1 rounded hover:bg-secondary transition-colors" title="Sil" onClick={() => void handleDeleteFile(file)}>
                            <Trash2 className="w-3 h-3 text-muted-foreground hover:text-red-300" />
                          </button>
                        </div>
                      </td>
                    </tr>
